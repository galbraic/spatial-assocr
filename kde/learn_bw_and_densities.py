...
bw_train_pop, bw_train_indv = (
    np.ones((train.shape[0], k.shape[0])),
    np.ones((train.shape[0], k.shape[0])),
)

density_train_pop, density_train_indv = (
    np.ones((train.shape[0], k.shape[0])),
    np.ones((train.shape[0], k.shape[0])),
)
density_val_pop, density_val_indv = (
    np.ones((val.shape[0], k.shape[0])),
    np.ones((val.shape[0], k.shape[0])),
)
density_test_pop, density_test_indv = (
    np.ones((test.shape[0], k.shape[0])),
    np.ones((test.shape[0], k.shape[0])),
)
...
for nn in k:
    ...
    bw_train_pop[:, c] = kde_2d._learn_nearest_neighbors_bandwidth(
        train[:, 1:3], nn, KM_TO_LON, KM_TO_LAT
    )  # calc population bw
    ...
    for user in range(uid.shape[0]):
        train_index, val_index, test_index = (
            np.where(train[:, 0] == uid[user]),
            np.where(val[:, 0] == uid[user]),
            np.where(test[:, 0] == uid[user]),
        )
        train_user = train[train_index[0], :]

        bw_train_indv[train_index[0], c] = kde_2d._learn_nearest_neighbors_bandwidth(
            train_user, nn, KM_TO_LON, KM_TO_LAT
        )  # calc indv bw
        indv = np.append(
            train_user, np.reshape(bw_train_indv[train_index[0], c], (len(train_user), 1)), 1
        )
